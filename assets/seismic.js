...
const SEIS = (function () {
  ...
  /**
   * A lithology column: the rock model itself, drawn against two-way time so a
   * reader can see where the layers are relative to the wiggles.
   * bands: [{ t0, t1, name, fill }] in seconds. Draws bottom-up in order.
   */
  function lithColumn(ctx, rect, bands, tMin, tMax, opts) {
    const o = opts || {};
    const Y = (t) => rect.y + ((t - tMin) / (tMax - tMin)) * rect.h;
    ctx.save();
    ctx.beginPath(); ctx.rect(rect.x, rect.y, rect.w, rect.h); ctx.clip();
    if (o.background !== false) {
      ctx.fillStyle = o.background || 'rgba(22,25,28,0.06)';
      ctx.fillRect(rect.x, rect.y, rect.w, rect.h);
    }
    bands.forEach((b) => {
      const y0 = Y(b.t0), y1 = Y(b.t1);
      if (y1 < rect.y || y0 > rect.y + rect.h) return;
      ctx.fillStyle = b.fill;
      ctx.fillRect(rect.x, y0, rect.w, Math.max(1, y1 - y0));
      ctx.strokeStyle = 'rgba(255,255,255,0.85)';
      ctx.lineWidth = 1;
      ctx.beginPath(); ctx.moveTo(rect.x, y0); ctx.lineTo(rect.x + rect.w, y0); ctx.stroke();
      if (b.name && y1 - y0 > 12 && rect.w > 30) {
        ctx.font = '9px "IBM Plex Mono", monospace';
        ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
        ctx.fillStyle = b.ink || 'rgba(22,25,28,0.75)';
        ctx.fillText(b.name, rect.x + rect.w / 2, (y0 + y1) / 2);
      }
    });
    ctx.restore();
    frame(ctx, rect);
